...
class TbEditableSaver extends CComponent
{
	...
	public function update()
	{
		...
		*/
        ...
        if (isset($this->model->nested) && $this->model->saveNode(false, $this->changedAttributes)) {
            $this->afterUpdate();
        }
		elseif(count($this->changedAttributes)==1&&method_exists($this->model,"getIsRelatedAttribute")&&$this->model->getIsRelatedAttribute($this->changedAttributes)&&$this->model->saveEditedRelation($this->changedAttributes)) {

			$this->afterUpdate();
		}
		elseif ($this->model->save(false, $this->changedAttributes)) {
