...
<div class="checkBoxGroup row">
    ...
    if ($type=="tabs") {
        ...
        foreach ($params as $k => $param) {
            $tabs[] = array(
                "label"=>$param['name'],
                "content"=>$this->render("table", array("module"=>$module, "controller"=>$controller, "action"=>$action,"params"=>$params[$k]+["id"=>$k], "type"=>""), true),
                "active"=>$i==0
            );
            $i++;
        }
        ...
        if (isset($params['withEvent'])&&$params['withEvent']) {
            ...
            foreach ($params['params'] as $k=>$param) {
                ...
                $columns[] = array(
                    "header"=>"$param",
                    "type"=>"raw",
                    "value"=>"CHtml::checkBox('$column'.'[event_id]['.\$data['id'].']', ".($this->role->narrow == Role::LEVEL_PARENT && $this->parentIsAdmin && !$isUser? "true":"Yii::app()->controller->getInArray('$column'.'[event_id]['.\$data['id'].']', '$this->accesses')").", ".(!$this->parentIsAdmin ? "array(
                ...
                <div class="col-lg-<?=$width?>">
                    ...
                    if (isset($param['params']) && is_array($param['params']))
                        $this->getWidget($param['type'], $column, $param['params'], (!$this->parentIsAdmin ? array(
                            "disabled"=>!$this->controller->getInArray(stripslashes($compareName), $this->parentEventAccess)
                        ) : array()), ($this->role->narrow == Role::LEVEL_PARENT && $this->parentIsAdmin && !$isUser? :$this->controller->getInArray(stripslashes($compareName),$this->accesses)));
